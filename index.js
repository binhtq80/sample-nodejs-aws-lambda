...
const SERVICE_WSDL = process.env.SERVICE_WSDL || "http://ip-172-31-100-162.ap-southeast-1.compute.internal/ws/countries.wsdl";

const transformRequest = async(event)=>{
    let targetRequest = {};
    targetRequest.name = event.countryName; 
...
const callSOAPService = async(targetRequest, wsdl)=>{
    return new Promise((resolve, reject)=>{
        ...
        var options = {
            forceSoap12Headers: false // Required if soap12 bindings are to be used
        };
        ...
        soap.createClient(wsdl, options, function(err, client) {
            ...
            //client.describe();
            client.getCountry(targetRequest, function(err, result) {
                if(err){
                    reject(err);
                }
                else{
